...
function logger(func, logFunc) {
  return (...args) => {
    const log = args.reduce(
      (ac, el) => `${JSON.stringify(ac)},${JSON.stringify(el)}`,
    );
...
function partialUsingArguments(fn, ...args1) {
  const argsArray = [];
  argsArray.push(...args1);
  const resultFn = (...args2) => {
    argsArray.push(...args2);
    if (fn.length === argsArray.length) return fn(...argsArray);
    return resultFn;
  };
  return resultFn;
}
